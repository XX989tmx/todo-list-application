const setAsTodayIconsList:NodeListOf<Element> = document.querySelectorAll(".setAsTodayIcon");
console.log("today");
...
for (let i = 0; i < setAsTodayIconsList.length; i++) {
  ...
  setAsTodayIcon.addEventListener("click", (event) => {
    ...
    const LiElement: HTMLElement =
      setAsTodayIcon.parentElement?.parentElement?.parentElement;
